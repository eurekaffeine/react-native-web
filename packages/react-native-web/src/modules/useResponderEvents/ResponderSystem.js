...
export default class ResponderSystem {
  ...
  _eventListener = (domEvent: any) => {
    ...
    const isStartEvent =
      isStartish(eventType) && isPrimaryPointerDown(domEvent);
    ...
    const responderEvent = createResponderEvent(
      domEvent,
      this._responderTouchHistoryStore
    );
    ...
    if (isStartEvent || isMoveEvent || isEndEvent) {
      ...
      this._responderTouchHistoryStore.recordTouchTrack(
        eventType,
    ...
    if (
      ...
      if (eventPaths != null) {
        ...
        wantsResponder = this._findWantsResponder(
          eventPaths,
          domEvent,
          responderEvent
        );
    ...
    if (
      this._currentResponder.id != null &&
      this._currentResponder.node != null
    ) {
  ...
  _attemptTransfer(
    ...
    const { onResponderGrant, onResponderReject } =
      this._getResponderConfig(id);
    ...
    // Negotiate with current responder
      ...
      const { onResponderTerminate, onResponderTerminationRequest } =
        this._getResponderConfig(currentId);
  ...
  attachListeners() {
    if (canUseDOM && this._window.__reactResponderSystemActive == null) {
      ...
      this.documentEventsBubblePhase.forEach((eventType) => {
        this._window.document.addEventListener(eventType, this._eventListener);
      });
      this.documentEventsCapturePhase.forEach((eventType) => {
        this._window.document.addEventListener(
          eventType,
          this._eventListener,
          true
        );
